...
namespace Content.Client.OptionsMenu
{
	...
	public sealed partial class GraphicsTab : Control
	{
		...
		private readonly List<WindowMode> _windowModes = new() { WindowMode.Windowed, WindowMode.Fullscreen };
		...
		public GraphicsTab()
		{
			...
			foreach (var mode in _windowModes)
			{
				_windowModeRadio.AddButton($"{mode}", (int) mode);
			}
		...
		public void UpdateButtons()
		{
			...
			var setSizeRadio = _uiSizeRadio.SetValue(_cfg.GetCVar(CVars.DisplayUIScale));
			DebugTools.Assert(setSizeRadio);

			var setWindowRadio = _windowModeRadio.SetValue(_cfg.GetCVar(CVars.DisplayWindowMode));
			DebugTools.Assert(setWindowRadio);
